...
export function Card({
  ...
  return (
    <div
      className={`rounded-xl border border-border bg-white/90 shadow-card backdrop-blur dark:bg-black/40 ${className}`}
    >
      {children}
    </div>
  );
...
export function Button({
  ...
  const base =
    "inline-flex items-center justify-center rounded-md px-3 sm:px-4 py-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50";
  const variants: Record<string, string> = {
    ...
    outline:
      "border border-border text-[hsl(var(--fg))] hover:bg-black/5 dark:hover:bg-white/10",
  };
  return (
    <button
      type={type}
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full rounded-md border border-border bg-white/70 px-3 py-2 text-sm sm:text-base dark:bg-black/30 ${
        props.className ?? ""
      }`}
    />
  );
...
export function Toolbar({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 rounded-xl border border-border bg-white/60 p-3 dark:bg-black/20">
      {children}
    </div>
  );
}
...
export function Hero() {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-sky-50 to-indigo-100 p-4 sm:p-6 dark:from-indigo-950/30 dark:to-sky-950/20">
      <div className="relative z-10">
        <h3 className="text-lg sm:text-xl md:text-2xl font-semibold">
          Pesquisa unificada & conexões
        </h3>
        <p className="mt-2 max-w-2xl text-sm sm:text-base text-muted">
          Encontre rapidamente a pessoa certa, com ranking explicável,
          normalização de lugares e um clique para abrir no FamilySearch.
        </p>
      </div>
  ...
  React.useEffect(() => {
    const saved =
      typeof window !== "undefined"
        ? window.localStorage.getItem("ga_theme")
        : null;
    const isDark = saved
      ? saved === "dark"
      : window.matchMedia &&
