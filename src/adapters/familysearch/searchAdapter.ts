...
import { collectPlacesFromGedcom, mapGedcomPerson, normalizeRef } from './utils';
...
export class FamilySearchSearchAdapter implements SearchAdapter {
  ...
  async searchPersons(params: SearchParams): Promise<MatchCandidate[]> {
    try {
      ...
      const data = await client.get<any>(`/platform/tree/search?${query.toString()}`, {
      ...
      for (const entry of entries) {
        ...
        const primaryPerson = personsRaw.find((p: any) => p?.principal === true) ?? personsRaw[0];
        const person = mapGedcomPerson(primaryPerson, { placesById: placeCache });
        if (!person) continue;
        if (!person.fsUrl) {
      ...
      throw new Error(`Falha ao buscar pessoas no FamilySearch: ${(err as Error).message}`);
    }
  }
