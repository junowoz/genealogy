...
export class FamilySearchChangeAdapter implements ChangeAdapter {
  ...
  async getPersonChanges(personId: string, cursor?: string): Promise<{ changes: ChangeItem[]; nextCursor?: string }> {
    try {
      ...
      const data = await client.get<any>(
        `/platform/tree/persons/${encodeURIComponent(normalizeRef(personId) ?? personId)}/changes${suffix}`
      );
      const entries = extractEntries(data);
      const changes = entries.map((entry: any) => mapChangeEntry(entry, personId)).filter(Boolean) as ChangeItem[];
      ...
      throw new Error(`Falha ao carregar alterações: ${(err as Error).message}`);
    }
  }
...
function mapChangeEntry(entry: any, personId: string): ChangeItem | undefined {
  ...
  const id =
    normalizeRef(entry?.id ?? entry?.identifier ?? entry?.changeId ?? entry?.links?.self?.href) ??
    entry?.id ??
    entry?.identifier ??
    undefined;
  const summary = entry?.summary ?? entry?.title ?? entry?.category?.label;
  const updated = entry?.updated ?? entry?.published ?? entry?.timestamp ?? entry?.changed ?? new Date().toISOString();
  const contributor =
    entry?.author?.name ??
    entry?.agent?.name ??
  ...
  const fsChangeUrl =
    ...
    `https://www.familysearch.org/tree/person/details/${encodeURIComponent(personId)}/changes`;
...
function extractChangeContent(entry: any) {
  ...
  if (Array.isArray(changes) && changes.length) {
    ...
    return {
      ...
      field: primary?.field ?? primary?.data?.field ?? primary?.changedAttribute,
...
function extractCursor(href?: string) {
  ...
  try {
    ...
    const value =
      url.searchParams.get('after') ?? url.searchParams.get('cursor') ?? url.searchParams.get('next');
