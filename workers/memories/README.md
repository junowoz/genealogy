...
- The root defaults to the repository root, or `APP_STORAGE_DIR` when set.
...
3. Use the app to upload a file and create a job
  - POST /api/memories/upload (multipart/form-data: file; requires login)
  - POST /api/memories/jobs { uploadId } (requires login)
4. Poll job status
  - GET /api/memories/jobs/:id (requires login)
