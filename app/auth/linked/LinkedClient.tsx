...
export default function LinkedClient({ state }: { state: string }) {
  useEffect(() => {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-slate-50 px-6 text-center text-slate-800">
      <div className="rounded-3xl bg-white/80 px-8 py-10 shadow-lg">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-600">✓</div>
        ...
        <p className="mt-6 text-xs text-slate-500">Caso a janela não feche automaticamente, feche manualmente após confirmar no aplicativo.</p>
      </div>
      <a className="text-xs text-slate-500 underline underline-offset-4" href="/">Voltar para genealogy.junowoz.com</a>
    </main>
  );
}
