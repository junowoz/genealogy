...
async function createJob(uploadId: string) {
  const res = await fetch("/api/memories/jobs", {
    method: "POST",
...
export default function MemoriesPage() {
  ...
  const upMut = useMutation({
    ...
    onSuccess: (data) => setUploadId(data.upload.id),
  });
  const jobMut = useMutation({
    mutationFn: createJob,
    onSuccess: (data) => setJobId(data.job.id),
  });
  const { data: jobData, isFetching: jobFetching } = usePollingJob(
    jobId ?? undefined
  );
  ...
  return (
    <div className="space-y-8">
      <Section
        title="Memories AI Ingestor"
        description="Envie uma imagem/PDF e eu extraio nomes, datas e lugares; sugiro pessoas do Tree e crio uma citação (read‑only, com redirects para o FamilySearch)."
      >
        <div
          className={`relative mt-2 rounded-xl border-2 border-dashed p-8 text-center ${
            dragOver
              ? "border-[hsl(var(--accent))] bg-black/5 dark:bg-white/10"
              : "border-border"
          }`}
          onDragOver={(e) => {
            e.preventDefault();
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          {upMut.isPending || jobMut.isPending || jobFetching ? (
            <span className="text-sm text-muted">Processando…</span>
          ) : null}
          {job ? (
            <span className="text-sm text-muted">Status: {job.status}</span>
          ) : null}
        </div>
      ...
      {isDone && result ? (
        <Section
          title="Resultado"
          description="Extração (OCR/NER), sugestões de pessoas e citação sugerida."
        >
                ...
                {result.ner.entities.map((e) => (
                  <div key={e.id} className="rounded border border-border p-2">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1 sm:gap-0">
                      <div className="font-medium break-words">{e.text}</div>
                      <Badge
                        tone={
                          e.type === "PERSON_NAME"
                            ? "success"
                    ...
                    <div className="mt-1 text-xs text-muted">
                      confiança {(e.confidence * 100).toFixed(0)}% • pág{" "}
                      {e.page}
                    </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <div className="mt-1 flex flex-wrap gap-2">
                            {s.explanations.map((ex, j) => (
                              <Badge key={j}>{ex}</Badge>
                            ))}
                          </div>
                        </div>
            ...
            <Card className="p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="text-xs text-muted">
                  Anexar/confirmar deve ser feito na interface oficial do FS
                  (redirect).
                </div>
              </div>
            </Card>
